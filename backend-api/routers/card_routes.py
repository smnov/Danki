...
async def learning_status(id: int):
    ...
    cards_found = select(Card).where(Card.deck_id == id).where(Card.status == "learning")
...
async def repeat_status(id: int):
    ...
    cards_found = select(Card).where(Card.deck_id == id).where(Card.status == "repeat")
    res = session.exec(cards_found).all()
    if not res:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
    return res
...
async def new_cards(id: int):
    ...
    cards_found = select(Card).where(Card.deck_id == id).where(Card.status == "New")
    res = session.exec(cards_found).all()
    if not res:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
    return res
